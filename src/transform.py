...
if __name__ == "__main__":
    ...
    if args.transform:
        ...
        # Get all the image paths from data folder 
        ...
        # Get all the labels from data folder
        ...
        # Convert the dataset into dictionary type
        ...
        # Preprocessing images using our custom preprocess function
        pool = Pool()
        dataset['image'] = pool.map(partial(prep.preprocess_raw, input_size=INPUT_SIZE, save_type=args.type), dataset['image_path'])
        pool.close()
        pool.join()

        # Create a jason file of labels 
